...
### Решение 3
* Создал файл vms_platform.tf. Перенес в него переменные VM1 и указал переменные для VM2
...
* т.к. ВМ2 в другой зоне (отличной от зоны по умолчанию, пришлось добавлять подсеть зоны B).
...
* Команда terraform plan показала изменения (создание VM2)
...
* locals.tf описал локальные переменные
...
5. Найдите и закомментируйте все, более не используемые переменные проекта.
